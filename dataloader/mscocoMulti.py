...
class MscocoMulti(data.Dataset):
    ...
    def __getitem__(self, index):
      ...
      #  image_name = '00000786.jpg'
        ...
        if self.is_train:
            points = np.array(a['unit']['keypoints']).reshape(self.total_keypoints, 3).astype(np.float32)
        ...
        if self.is_train:
            ...
            #        print(pts[i])
                    target15[0] = generate_heatmap(target15[0], pts[i], self.cfg.gk15)
                    target11[0] = generate_heatmap(target11[0], pts[i], self.cfg.gk11)
                    target9[0] = generate_heatmap(target9[0], pts[i], self.cfg.gk9)
                    target7[0] = generate_heatmap(target7[0], pts[i], self.cfg.gk7)
