...
def train(train_loader, model, criterions, optimizer):
	...
	#print(i)
     ...
     #   print(target7.shape)
     #   print(11111111)
     #   plt.imshow(target7[0][0], cmap='hot', interpolation='nearest')
     #   plt.show()
        refine_target_var = torch.autograd.Variable(target7.cuda(async=True))
        #print(refine_target_var.size())
        ...
        for global_output, label in zip(global_outputs, targets):
         ...
         #   print((valid > 1.1).type(torch.FloatTensor).view(-1, num_points, 1, 1).size())
         #   print(valid.size())
      ...
      #  refine_loss *= (valid_var > 0.1).type(torch.cuda.FloatTensor)
        refine_loss = ohkm(refine_loss, 1)  #8
        loss += refine_loss
